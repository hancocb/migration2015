...
land = form.getvalue('land_list').split(' ')
soil_list = form.getvalue('soil_list').split(' ')
area_sum = form.getvalue('area_sum').split(' ')
land_list = [[0 for x in range(8)] for x in range(numWSD)]
for i in range(0,numWSD):
	for j in range(0,8):
		land_list[i][j] = float(land[(i*8)+j])
...
#-----------------------------------------------------------------------------------------------------------------------Step1
# print ' Debug <input type=text size=100 value="',land,'"> </br></br>'
...
for i in range(1,numWSD+1) :
  ...
  for j in range(1,9) : 
    ...
    tmpStr = tmpStr + ' size=5  onChange="javascript:' + js_totArea +'()" value='+str(land_list[i-1][j-1])+'>'
...
print '<div style="text-align:center"><input style="WIDTH:200px;height:30px;CURSOR:hand" type=button onClick="javascript:window.open(\'https://engineering.purdue.edu/mapserve/ldctest/STEPL/\')" value="Restart"> '
